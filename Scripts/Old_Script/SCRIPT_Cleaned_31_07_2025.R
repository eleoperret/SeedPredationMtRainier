...
data_ae10 <- data_ae10 %>%
  mutate(
    Time_clean = hms::as_hms(format(Time, "%H:%M:%S")),  # Extract time only
    Date_clean = as.Date(Date),                          # Ensure Date is date class
    DateTime = as.POSIXct(paste(Date_clean, Time_clean), tz = "UTC") # Combine date + clean time
  )
on_tray_data <- data_ae10 %>%
  filter(On_tray == "On", Species_ID %in% c("Tamias sp", "Peromyscus maniculatus")) %>%
  arrange(Species_ID, Camera, Date_clean, Time_clean)
# Assign visit ID using rleid from data.table (consecutive groupings)
library(data.table)

on_tray_data_dt <- as.data.table(on_tray_data)
on_tray_data_dt[, visit_id := rleid(Species_ID, Camera, Depression_number, Date, Time > shift(Time, fill = Time[1]) + 30), by = .(Species_ID, Camera)]

# Estimate number of images per visit
visit_summary <- on_tray_data_dt[, .(
  n_images = .N,
  start_time = min(Time),
  end_time = max(Time)
), by = .(Species_ID, Camera, visit_id)]

# Add estimated time (assuming ~3 sec per image)
visit_summary[, duration_sec := as.numeric(difftime(end_time, start_time, units = "secs"))]

# Get total time per species
total_time <- visit_summary[, .(
  total_duration_sec = sum(duration_sec, na.rm = TRUE),
  n_visits = .N
), by = Species_ID]

# Show time in minutes
total_time$total_duration_min <- round(total_time$total_duration_sec / 60, 1)

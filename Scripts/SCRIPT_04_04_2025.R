...
glmer_model <- glmer(cbind(Success, Seeds.Placed - Success) ~ Stand + Treatment + Seed_sp  + (1|Camera) + (1|Week), 
                 data = data_cleaned_2, 
...
#Trying different models (adding and removing the explanatory variables: Seed sp. /Stand and Treatment.+ adding some random variables)
glmer_model2 <- glmer(cbind(Success, Seeds.Placed - Success) ~ 1 + (1|Camera) + (1|Week), 
                     data = data_cleaned_2, 
                     family = binomial(link = "logit"))
glmer_model3 <- glmer(cbind(Success, Seeds.Placed - Success) ~ Seed_sp + (1|Camera) + (1|Week), 
                      data = data_cleaned_2, 
                      family = binomial(link = "logit"))
glmer_model4 <- glmer(cbind(Success, Seeds.Placed - Success) ~ Stand + (1|Camera) + (1|Week), 
                      data = data_cleaned_2, 
                      family = binomial(link = "logit"))
glmer_model5 <- glmer(cbind(Success, Seeds.Placed - Success) ~ Stand + Treatment + (1|Camera) + (1|Week), 
                      data = data_cleaned_2, 
                      family = binomial(link = "logit"))
glmer_model6 <- glmer(cbind(Success, Seeds.Placed - Success) ~ Stand + Treatment + Seed_sp + (1|Camera) + (1|Week), 
                      data = data_cleaned_2, 
                      family = binomial(link = "logit"))
glmer_model7 <- glmer(cbind(Success, Seeds.Placed - Success) ~ Stand +  Seed_sp + (1|Camera) + (1|Week), 
                      data = data_cleaned_2, 
                      family = binomial(link = "logit"))
glmer_model8 <- glmer(cbind(Success, Seeds.Placed - Success) ~ Stand + Treatment  + (1|Camera) + (1|Week), 
                      data = data_cleaned_2, 
                      family = binomial(link = "logit"))
glmer_model9 <- glmer(cbind(Success, Seeds.Placed - Success) ~ Treatment + Seed_sp + (1|Camera) + (1|Week), 
                      data = data_cleaned_2, 
                      family = binomial(link = "logit"))
glmer_model10 <- glmer(cbind(Success, Seeds.Placed - Success) ~ Stand + Treatment + Seed_sp + (1|Camera) + (1|Week), 
                      data = data_cleaned_2, 
                      family = binomial(link = "logit"))
glmer_model11 <- glmer(cbind(Success, Seeds.Placed - Success) ~ Stand * Treatment + Seed_sp +  (1|Camera) + (1|Week), 
                      data = data_cleaned_2, 
                      family = binomial(link = "logit"))
glmer_model12 <- glmer(cbind(Success, Seeds.Placed - Success) ~ Stand + Treatment * Seed_sp + (1|Camera) + (1|Week), 
                       data = data_cleaned_2, 
                       family = binomial(link = "logit"))
#glmer_model13 <- glmer(cbind(Success, Seeds.Placed - Success) ~ Stand * Treatment * Seed_sp + (1|Camera) + (1|Week), data = data_cleaned_2, family = binomial(link = "logit"))
#This last model cannot work.
...
models <- list(
  ...
  model8  = glmer_model8,
  model9  = glmer_model9,
  model10 = glmer_model10,
  model11 = glmer_model11,
  model12 = glmer_model12
)
...
cat("\nModel with lowest AIC:", best_model_name, "with AIC =", best_aic, "\n")

#Checking my model
summary(glmer_model11)
plot(glmer_model11)
#install.packages("DHARMa")
library(DHARMa)
# Simulate residuals
sim_res <- simulateResiduals(fittedModel = glmer_model11, plot = TRUE)
# Simulate residuals
sim_res <- simulateResiduals(glmer_model11)
# Check dispersion
...
overdisp_fun(glmer_model11)
#Yes there is.

#Checking if I should have or not random effects in my model
# Fitting the model with random effects
model_with_random <- glmer(cbind(Success, Seeds.Placed - Success) ~ Stand * Treatment + Seed_sp + (1|Camera) + (1|Week), 
                           data = data_cleaned_2, 
                           family = binomial(link = "logit"))
# Fitting the model without random effects
model_without_random <- glm(cbind(Success, Seeds.Placed - Success) ~ Stand * Treatment + Seed_sp, 
                              data = data_cleaned_2, 
                              family = binomial(link = "logit"))
# Perform Likelihood Ratio Test
anova(model_without_random, model_with_random)
# Compare AIC
AIC(model_with_random, model_without_random)
#The model is not better without the randow effects. 
...
glmer_modelOLRE <- glmer(
  cbind(Success, Seeds.Placed - Success) ~ Stand * Treatment + Seed_sp +
...
#Doing a model without the interaction
...
anova (glmer_modelOLRE,glmer_modelOLRE_2)
summary(glmer_modelOLRE_2)
#this model is better (but no more significance for treatment)

#Model without treatment
glmer_modelOLRE_3 <- glmer(
  cbind(Success, Seeds.Placed - Success) ~ Stand + Seed_sp +
    (1 | Camera) + (1 | Week) + (1 | ObsID),
  family = binomial,
  data = data_cleaned_2
)
anova (glmer_modelOLRE_2,glmer_modelOLRE_3)
#Slightly better AIC score

summary(glmer_modelOLRE_3)
plot(glmer_modelOLRE_3)
vif(glmer_modelOLRE_3)  # Check for multicollinearity

# Simulate residuals
sim_res <- simulateResiduals(fittedModel = glmer_modelOLRE_3, plot = TRUE)
# Simulate residuals
sim_res <- simulateResiduals(glmer_modelOLRE_3)
#Much better!

#Overall the Stand + Seed_sp is the best model. 
...
model1ABAM <- glmer(cbind(Success, Seeds.Placed - Success) ~ 1 + (1|ObsID), 
                data = ABAM, 
                family = binomial(link = "logit"))
model2ABAM <- glmer(cbind(Success, Seeds.Placed - Success) ~ Stand + (1|ObsID), 
                data = ABAM, 
                family = binomial(link = "logit"))
model3ABAM <- glmer(cbind(Success, Seeds.Placed - Success) ~ Treatment+ (1|ObsID), 
                data = ABAM, 
                family = binomial(link = "logit"))
model4ABAM <- glmer(cbind(Success, Seeds.Placed - Success) ~ Stand + Treatment+ (1|ObsID), data = ABAM, 
                family = binomial(link = "logit"))
model5ABAM <- glmer(cbind(Success, Seeds.Placed - Success) ~ Stand * Treatment+ (1|ObsID), data = ABAM, 
                family = binomial(link = "logit"))
...
plot(model2ABAM)
summary(model2ABAM)
sim_res <- simulateResiduals(fittedModel = model2ABAM, plot = TRUE)

# ABLA --------------------------------------------------------------------
ABLA<-data_cleaned_2%>%
  filter(Seed_sp=="ABLA")
ABLA$ObsID <- factor(1:nrow(ABLA))
model1ABLA <- glmer(cbind(Success, Seeds.Placed - Success) ~ 1+ (1|ObsID), 
                  data = ABLA, 
                  family = binomial(link = "logit"))
model2ABLA <- glmer(cbind(Success, Seeds.Placed - Success) ~ Stand+ (1|ObsID), 
                  data = ABLA, 
                  family = binomial(link = "logit"))
model3ABLA <- glmer(cbind(Success, Seeds.Placed - Success) ~ Treatment+ (1|ObsID), 
                  data = ABLA, 
                  family = binomial(link = "logit"))
model4ABLA <- glmer(cbind(Success, Seeds.Placed - Success) ~ Stand + Treatment+ (1|ObsID), data = ABLA, 
                  family = binomial(link = "logit"))
model5ABLA <- glmer(cbind(Success, Seeds.Placed - Success) ~ Stand * Treatment+ (1|ObsID), data = ABLA, 
                  family = binomial(link = "logit"))
...
# PSME --------------------------------------------------------------------
#Not possible to use a glmer here because creates non-conerging models due to the high number of 1 (sucess) so I'm using a glm here. Does it make sense then ?
PSME<-data_cleaned_2%>%
  filter(Seed_sp=="PSME")
PSME$ObsID <- factor(1:nrow(PSME))
plot(PSME$seeds)
ggplot(PSME, aes(x = Stand, y = Success / Seeds.Placed, fill = Treatment)) +
  geom_boxplot() +
  theme_minimal()


model1PSME <- glm(cbind(Success, Seeds.Placed - Success) ~ 1 , 
                  data = PSME, 
                  family = binomial(link = "logit"))
model2PSME <- glm(cbind(Success, Seeds.Placed - Success) ~ Stand, 
                  data = PSME, 
                  family = binomial(link = "logit"))
model3PSME <- glm(cbind(Success, Seeds.Placed - Success) ~ Treatment, 
                  data = PSME, 
                  family = binomial(link = "logit"))
model4PSME <- glm(cbind(Success, Seeds.Placed - Success) ~ Stand + Treatment, data = PSME, 
                  family = binomial(link = "logit"))
model5PSME <- glm(cbind(Success, Seeds.Placed - Success) ~ Stand * Treatment, data = PSME, 
                  family = binomial(link = "logit"))
...
plot(model5PSME)
summary(model5PSME)
sim_res <- simulateResiduals(fittedModel = model5PSME, plot = TRUE)
#Model 5 (Stand * Treatment) has the lowest AIC score but when looking at the summary nothing is significant. 
